...
test('active guidance leads with the memory itself and keeps compact canonical sections', () => {
  ...
  assert.match(source, /Always avoid[\s\S]*Firm rules your stylist always follows\. These rules only apply in the situations shown\. Everything else stays available\./)
  assert.match(source, /Specific pieces[\s\S]*Pieces your stylist avoids only in the situations shown\. Outside those situations, they stay in the mix\./)
...
test('active guidance uses effect language instead of storage labels', () => {
  ...
  assert.match(source, /Skip for <em>\{joinWithAnd\(group\.entries\.map/)
  assert.match(source, /Still available for other occasions/)
  assert.match(source, /Still available elsewhere/)

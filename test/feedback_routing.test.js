...
test('feedback routing assigns one primary behavioural reader to each reaction family', () => {
  const cases = [
    ...
    [{ feedback_type: 'wrong_item_read', target_type: 'whole_wardrobe_outfit' }, FEEDBACK_BEHAVIOURS.STYLING_PROMPT],
    [{ feedback_type: 'wrong_item_read', target_type: 'whole_wardrobe_outfit', payload: JSON.stringify({ scopedEvidence: { version: 1, kind: 'garment_context_suitability', subjectPieceId: 42 } }) }, FEEDBACK_BEHAVIOURS.CONTEXT_SCORE],
    [{ feedback_type: 'wrong_item_read', target_type: 'whole_wardrobe_outfit', payload: JSON.stringify({ scopedEvidence: { version: 1, kind: 'garment_context_suitability', subjectPieceId: null } }) }, FEEDBACK_BEHAVIOURS.STYLING_PROMPT],

...
test('StylistChat new-chat empty state prioritizes the freeform composer', () => {
  ...
  assert.match(src, /I can create outfits from your saved pieces or review an outfit photo\./)
  ...
  assert.match(css, /\.stylist-empty-state[\s\S]*width: min\(100%, 920px\)/)
  assert.match(css, /\.stylist-empty-state[\s\S]*margin: clamp\(46px, 9vh, 104px\) auto 28px/)
  ...
  assert.match(css, /\.composer-create-outfits-btn[\s\S]*background: transparent/)
...
test('Wardrobe page keeps primary filters visible and collapses color and fabric controls', () => {
  ...
  const css = fs.readFileSync(path.join(process.cwd(), 'src/App.css'), 'utf8')
  assert.match(inventory, /className="wardrobe-filter-label">Category/)
  assert.match(inventory, /className="wardrobe-filter-label">Occasion/)
  assert.match(inventory, /className="wardrobe-filter-label">Season/)
  assert.match(inventory, /filterColor \? 'Color · 1' : 'Color'/)
  assert.match(inventory, /filterFabric \? 'Fabric · 1' : 'Fabric'/)
  assert.match(inventory, /className="filter-menu-chevron"/)
  assert.match(inventory, /className="wardrobe-color-grid"/)
  ...
  assert.match(inventory, />\s*Add piece\s*<\/button>/)
  assert.doesNotMatch(inventory, />Colors:<\/span>/)
  assert.doesNotMatch(inventory, />Fabrics:<\/span>/)
  assert.match(pieceCard, /role="button"/)
  assert.match(pieceCard, /tabIndex=\{0\}/)
  assert.match(pieceCard, /aria-label=\{`Open \$\{piece\.name\} wardrobe card`\}/)
  assert.match(pieceCard, /event\.target\.closest\('button'\)/)
  assert.match(pieceCard, /className="piece-card-id"/)
...
test('Opened garment modal uses grouped metadata and updated detail hierarchy', () => {
  ...
  assert.match(detail, /Saved boards · \{savedBoards\.length\}/)
  ...
  assert.match(detail, /aria-label=\{`Show \$\{remainingSavedBoards\} more saved boards`\}/)

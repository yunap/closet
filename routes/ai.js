...
2. INDEPENDENT WEARABILITY. Prefer pieces that work on their own. A piece that always needs something else under or over it costs two places to produce one look — it and its base each take a separate slot out of this capsule's fixed budget, not one shared slot — so it has to earn that cost with a look the capsule could not otherwise make. When you take one, its base must be a genuine visual match, not merely present: check opacity and coverage (an open-weave or sheer base does not conceal what it needs to), neckline and strap or sleeve shape, length, bulk, and colour relationship — whether the base is meant to stay hidden or to show intentionally as part of the look. "A tank exists in the roster" is not sufficient; the tank has to actually sit right under that particular construction. Weigh which kind of dependency this is: a strong one, where the base also works alone, supports another piece, and appears in other looks — a connector with multiple jobs, easily worth the second slot — versus a weak one, where the base exists only to make this one piece wearable, is never shown alone, and produces only a look or two. A weak dependency is not automatically wrong, but it has to earn its cost through unusual visual strength, useful context coverage, or a role nothing else in the set fills — not assumed. If you take more than one dependent piece, make sure they lead to genuinely different outfits — different silhouette, different context, or a different base — rather than the same combination twice.

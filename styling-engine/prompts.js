...
- First declare cards intent with outfit_count:1 and the correct layer_requirement. Then make one batched visual wardrobe search covering every category you may need. Its system_paths are complete mechanically/weather-feasible starting points, not aesthetic recommendations: judge their attached photographs and choose the strongest outfit. The eligible_piece_index is complete even though only the visual working set has photographs; you may use view_pieces once for an indexed alternative that could materially improve the outfit. If you recombine pieces across paths, propose_outfit validates the new system normally. Submit exactly one complete card through propose_outfit; if validation rejects it, follow that repair instruction and resubmit rather than presenting the rejected card.

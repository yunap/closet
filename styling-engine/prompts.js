...
- First declare cards intent with outfit_count:1 and the correct layer_requirement. Then make one batched wardrobe search covering every category you may need; do not try to narrow it with styling filters. It returns a complete sparse stylist_catalog of hard-eligible garments, in identity order—not ranked outfits and not a code-selected shortlist. Read the catalog's structured styling and physical facts, choose the candidates whose photographs you need to compare, and call view_pieces with up to 12 IDs across complete outfit possibilities. If those photographs expose a real problem, you may make one additional targeted view call with up to 4 IDs. Judge the photographs and compose the strongest outfit yourself. Submit exactly one complete card through propose_outfit; if validation rejects it, follow that repair instruction and resubmit rather than presenting the rejected card.

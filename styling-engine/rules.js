...
export function locallyGateWholeWardrobeOutfits(outfits = [], limit = 5, { mode = 'gate', requireShoes = true, requireDress = false, requireNonGraphicTop = false, rejectProfileDiscouraged = false, applyDiversity = true, repair = undefined, candidatePieces = [], occasion = 'casual', mood = '', season = '', weatherProfile = null, activity = '', sessionInfluence = null, request = '', question = '' } = {}) {
  ...
  for (const outfit of outfits) {
    ...
    // [O2]: the whole-wardrobe wrapper owns a resolved profile, so it feeds the shared Contract C
    // stage. Deliberately `weatherProfile`, NOT `resolvedWeatherProfile` — the latter falls back to
    // weatherProfileFromContext({ mood, season }), which is a local derivation from prose. That
    // fallback is fine for this wrapper's own ranking and repair, but Contract C must run on

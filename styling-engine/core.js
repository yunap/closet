...
export async function buildStylistConversationPayload(body) {
  ...
  const system = STYLIST_SYSTEM + [
    ...
    'If mode is new_request and required context (both location/city and weather/season/dates) is present, answer the user’s request directly using wardrobe context. Suggest specific wardrobe pieces in a natural conversational paragraph. You must NOT use bullet points, numbered lists, or bold label prefixes (such as Top, Bottom, Shoes, Bag) to structure the outfit parts (unless it is an explicit packing list where list format is acceptable, but even then you must recommend specific closet pieces by their actual name). If details like location/city, weather, season, or dates are missing, do not suggest an outfit, recommend garments, or generate a list; you must ask exactly one clear clarifying question to gather this context.',

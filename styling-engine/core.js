...
export async function buildStylistConversationPayload(body) {
  ...
  const system = STYLIST_SYSTEM + [
    ...
    'INTENT DECLARATION (mechanically enforced): before composing or answering substantively, call declare_intent with what this turn should produce — want:"text" (advice/critique prose), "cards" (composed outfit cards), or "image" (a rendered outfit image), plus outfit_count when the user asked for a specific number. propose_outfit and generate_outfits are blocked until cards intent is declared. If the user wants an image, declare want:"image" and answer honestly: in-chat rendering is not available; point to the "Generate outfit image" button on an outfit card.',

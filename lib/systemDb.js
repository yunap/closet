...
// ── Invite requests (table + admin side shipped in PR A; the public unauthenticated
// submission endpoint is spec 34 PR B, wired through routes/inviteRequests.js) ──
export function createInviteRequest(email, note = '') {
  // The public endpoint is unauthenticated, so keep the row small and bounded: normalise
  // the email, cap the free-text note, and never trust length. Duplicate emails are
  // allowed on purpose — a second request is a signal, not an error, and the operator
  // decides from the admin queue either way.
  const cleanEmail = String(email || '').toLowerCase().trim().slice(0, 200)
  const cleanNote = String(note || '').trim().slice(0, 1000)
  const info = db.prepare('INSERT INTO invite_requests (email, note) VALUES (?, ?)').run(cleanEmail, cleanNote)
  return db.prepare('SELECT * FROM invite_requests WHERE id = ?').get(info.lastInsertRowid)
}

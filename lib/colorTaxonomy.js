export const COLOR_TAXONOMY = Object.freeze([
  ...
  { name: 'cognac', hex: '#8E4C32', family: 'brown', neutrality: 'neutral-adjacent' },
  { name: 'chocolate', hex: '#4A3222', family: 'brown', neutrality: 'neutral-adjacent' },
  ...
  { name: 'terracotta', hex: '#B85C45', family: 'orange', neutrality: 'accent' },
  { name: 'peach', hex: '#F2B38F', family: 'orange', neutrality: 'accent' },
  ...
  { name: 'emerald', hex: '#296B4F', family: 'green', neutrality: 'accent' },
...
// Everyday colour words that map cleanly to one of the stored taxonomy
// families. They are request-language aliases, not additional tagger output:
// the database remains canonical while a person can still ask for fuchsia.
export const COLOR_REQUEST_ALIASES = Object.freeze({
  fuchsia: 'pink',
  magenta: 'pink',
})

export const COLOR_REQUEST_NAMES = Object.freeze([
  ...COLOR_NAMES,
  ...Object.keys(COLOR_REQUEST_ALIASES),
])
...
export function colorTaxonomyEntry(value) {
  ...
  return COLOR_BY_NAME.get(COLOR_REQUEST_ALIASES[normalized] || normalized) || UNKNOWN_COLOR
}

export function colorFamilyLabel(family) {
  const normalized = normalizeColorName(family)
  return COLOR_FAMILY_LABELS[normalized] || normalized || 'Unknown'
}
...
export function partitionCanonicalColors(values = []) {
  const canonical = []
  const unknown = []
  for (const value of Array.isArray(values) ? values : []) {
    const normalized = normalizeColorName(value)
    if (!normalized) continue
    const destination = colorTaxonomyEntry(normalized).family === 'unknown' ? unknown : canonical
    if (!destination.includes(normalized)) destination.push(normalized)
  }
  return { canonical, unknown }
}

export function sanitizeTaggerColors(tags = {}, { preserveExisting = false } = {}) {
  const sanitized = { ...tags }
  if (!Object.prototype.hasOwnProperty.call(tags, 'colors')) {
    return { tags: sanitized, gaps: [] }
  }
  const rawColors = Array.isArray(tags.colors) ? tags.colors : []
  const { canonical, unknown } = partitionCanonicalColors(rawColors)
  if (preserveExisting && rawColors.length > 0 && canonical.length === 0) delete sanitized.colors
  else sanitized.colors = canonical
  sanitized.color_taxonomy_gaps = unknown
  return { tags: sanitized, gaps: unknown }
}

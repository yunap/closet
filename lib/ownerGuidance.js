...
const contextMatches = (wanted, request = {}) => {
  ...
  const requestValues = {
    ...
    seasons: resolveCalendarSeason(request.calendarSeason || request.season, request.currentDate),
  }
  ...
  if (wanted.weather.length) {
    ...
    const weatherFlags = {
      hot: Boolean(weather?.hot ?? weather?.isHot ?? weather?.is_hot),
      cold: Boolean(weather?.cold ?? weather?.isCold ?? weather?.is_cold),
      rainy: Boolean(weather?.rainy ?? weather?.isRainy ?? weather?.is_rainy),
      wet_exposure: Boolean(weather?.wet_exposure ?? weather?.isWetExposure ?? weather?.is_wet_exposure),
    }
    if (!wanted.weather.some(value => weatherFlags[value] || hasTerm(weatherText, value.replaceAll('_', ' ')))) return false
  }

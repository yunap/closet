...
// Mirrors provider.js's callOutcomeFromUsage. Kept local because this hook runs at the fetch
// boundary and must not import the engine.
function truncationOutcome(usage, response, payload) {
  if (response.ok && usage?.stopReason === 'max_tokens') {
    return { success: false, errorMessage: 'provider truncated the response at the output token cap (stop_reason: max_tokens)' }
  }
  return { success: response.ok, errorMessage: errorMessage(payload, response) }
}
    ...
    try {
      ...
      void clone.json()
        .then(payload => logAiCall({
          ...
          // A 200 whose body was truncated at the output cap is not a success — the caller fails to
          // parse it moments later, by which time this row is already written. Before 2026-09-02
          // that made truncation invisible in the spend log (docs/tagger-cost-spec.md §6f). The
          // provider's own stop reason is authoritative, so this needs no heuristic.
          ...truncationOutcome(normalizeProviderUsage(payload, provider, model), response, payload),
          latencyMs,
          // stopReason recorded alongside status so a truncation is measurable rather than only
          // inferable from the failure flag (docs/tagger-cost-spec.md §6f).
          context: { ...baseContext, status: response.status, stopReason: normalizeProviderUsage(payload, provider, model)?.stopReason ?? null },

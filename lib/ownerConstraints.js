...
// Request season selectors carry aliases ('warm', 'autumn') and an unresolved placeholder
// ('current season', the composer's default when the owner never picked one) that are not
// themselves season names. `referenceDate` is the date "current" should resolve against — pass
// the actual request/event date, not necessarily today, when resolving something that happened
// in the past (e.g. season context recorded on a stored reaction).
export function resolveSeasonTerm(rawSeason, referenceDate) {
  const requested = normalized(rawSeason)
  if (requested === 'warm') return 'summer'
  if (requested === 'autumn') return 'fall'
  if (requested !== 'current season') return requested
...
export function ownerConstraintApplies(row, pieceFacts = {}, requestContext = {}) {
  ...
  if (dimension === 'season') requested = resolveSeasonTerm(requested, requestContext.currentDate)

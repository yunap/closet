...
export function updateAiTelemetryContext(patch = {}) {
  ...
  if (!store) {
    // Diagnostic breadcrumb (2026-08-24): a live Batch Add run showed 3 of 5 sequential
    // /tag-piece calls losing their request-scoped telemetry context (flow, tagger_source, and
    // originalUrl all came back empty) between the route handler and the eventual provider call.
    // Four escalating local repros (plain sharp fan-out, real Express+multer+sharp, the actual
    // patched SDK transport, and a ~2.5s simulated model latency) all failed to reproduce it, so
    // the mechanism is still unconfirmed. If this fires, the loss already happened by the time
    // the ROUTE HANDLER tried to stage its patch — earlier than logAiCall's own read below — and
    // the stack trace pins the exact call site.
    console.warn('[ai-call-telemetry] updateAiTelemetryContext called with no active context — patch dropped:', patch, new Error().stack)
    return
  }
...
export async function logAiCall({
  ...
  try {
    ...
    // Same diagnostic as updateAiTelemetryContext's breadcrumb above — this is the OTHER end of
    // the same unconfirmed context-loss mechanism: no explicit flow was passed AND the
    // request-scoped ALS context came back with nothing usable, so this call is about to be
    // logged as 'unattributed' with an empty tagger_source even if it was a real /tag-piece call.
    if (!flow && resolvedFlow === 'unattributed') {
      console.warn('[ai-call-telemetry] logAiCall found no usable telemetry context — logging as unattributed:', { provider, model, callKind }, new Error().stack)
    }

import { getAiTelemetryContext, logAiCall } from './aiCallTelemetry.js'
...
function mockCallKind(system = '', result = null) {
  ...
  const flow = getAiTelemetryContext()?.flow
  if (flow === 'ask' || getAiTelemetryContext()?.originalUrl === '/api/ai/ask') return 'tool_loop'
  return 'text'
}

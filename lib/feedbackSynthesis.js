...
// 2026-08-31 review correction: this was Anthropic's own formula (tokens ≈ width px × height px /
// 750) with no provider parameter at all — since the preview/authorization call now resolves
// through stylistProviderOverride and can be Gemini, a synthesis batch with several images could
// authorize against a cost figure understated by ~4x (a 448px garment thumb: 268 Anthropic-formula
// tokens vs Gemini 3's real ~1120 tokens/image at that resolution per
// https://ai.google.dev/gemini-api/docs/media-resolution). `prepareWardrobeThumb` resizes with
// `fit: inside`, so actual pixel count never exceeds maxPx² — using the square as the Anthropic
// estimate is a deliberate upper bound, same "conservative ceiling" the text estimate already uses.
// Gemini's own tiling scheme doesn't scale with maxPx in the same simple way, so a flat, cited,
// conservative per-image figure is used instead rather than guessing at an unverified formula.
const GEMINI_IMAGE_TOKENS_FLAT = 1120
// No published-and-verified formula in this codebase yet for OpenAI vision tokens either — held
// deliberately higher than Anthropic's own formula so an unrecognized/OpenAI provider still errs
// toward overestimating rather than repeating the exact mistake this fix corrects.
const OPENAI_IMAGE_TOKENS_FLAT = 850
export function estimateImagePixelTokens(maxPx, provider = 'anthropic') {
  const px = Math.max(1, Number(maxPx) || 0)
  if (provider === 'gemini') return GEMINI_IMAGE_TOKENS_FLAT
  if (provider === 'openai') return OPENAI_IMAGE_TOKENS_FLAT

...
// Lightweight AI-quality verdicts (plan: quizzical-foraging-boot, Stage G) — good/questionable/bad
// on a piece's tags or a stylist reply, captured during ordinary use to accumulate real evidence
// about a routed model (see routes/ai.js's TAGGER_PROVIDER_OVERRIDE/STYLIST_PROVIDER_OVERRIDE).
// Deliberately excluded from STYLING_PROMPT_FEEDBACK_TYPES/RENDERER_FEEDBACK_TYPES/owner_rule below
// — this is a judgment about which MODEL produced a result, not a styling preference, and must
// never enter the styling-rule synthesis pipeline. feedbackBehaviour() falls through to
// DISPLAY_ONLY for it, same as any other unrecognized type.
export const MODEL_QUALITY_VERDICT_LABELS = [
  ['model_quality_good', 'Good'],
  ['model_quality_questionable', 'Questionable'],
  ['model_quality_bad', 'Bad'],
]

export const KNOWN_FEEDBACK_TYPES = Object.freeze([...new Set([
  ...MODEL_QUALITY_VERDICT_LABELS.map(([value]) => value),
...
const legacyPiece = piece => ({
  ...
  sleeve_length: String(piece?.sleeve_length || '').trim(),
  sleeve_shape: String(piece?.sleeve_shape || '').trim(),

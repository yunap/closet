...
| `neckline` | top, dress, outerwear | `V \| scoop \| crew \| boat \| mock \| turtleneck \| cowl \| off-shoulder \| square \| wrap \| halter \| strapless \| one-shoulder \| collared \| shawl \| other \| unknown` | yes | PieceForm, BatchAdd | `CONSTRUCTION_BY_CATEGORY.showNeckline` gates display. Extended 2026-08-14 — dropped the old `none` value per owner call (existing `none`-tagged pieces keep working, nothing validates this field against a Set — see "How safe is it to extend an enum" note below). `turtleneck` activates `necklineWarmth()` in `attributes.js`, which already had a dead `turtle` regex waiting for a real tagged value. Extended to `outerwear` 2026-08-29 — the tagger schema (`prompts.js`) already asked for `neckline` on every category, but outerwear had no `showNeckline` in either intake surface, so the value it returned (e.g. a cardigan's collar shape) was stored and fed to the stylist as garment truth with no way for the owner to ever see, confirm, or correct it — the low-confidence marker on it could never clear. `intakeReview.js`'s `lowConfidenceFields()` gate updated to match, so a low-confidence outerwear neckline now surfaces as a review chip like it already did for top/dress. |
...
| `tuck_behavior` | top | `tucks_anywhere \| tucks_with_structure \| wear_over_only` | **yes as of 2026-08-14** (previously tagger only rated confidence on this field, never produced a value — see `_confidence` vs schema split in `prompts.js`) | PieceForm; **BatchAdd added 2026-08-14** (was DB+gate-wired but had zero UI anywhere before this pass) | `prompts.js` schema. The field is the safe authority for automatic composition gates, but **not infallible evidence in a direct stylist conversation** (owner amendment 2026-08-19). A manual/high-confidence value is strong evidence; missing/low-confidence values may be inferred cautiously from fit-visible photos, cut, fabric, length, silhouette and the receiving waistband. Clear contradictory evidence may challenge a tag, but the answer must disclose the conflict rather than silently replacing stored truth. Hem shape alone never decides tuckability. This amends the 2026-08-15 “sole authority” wording without restoring the old hem-derived rule. |
...
| `fit_on_body` | clothing | `clings_stretchy \| clings_drapey \| skims \| hangs_straight \| drapes \| structured \| none` | yes | PieceForm, BatchAdd (added 2026-08-14 — was tagged and DB-wired but had no BatchAdd control before this pass) | `FIT_ON_BODY_VALUES`/`FIT_ON_BODY_SCHEMA_DESCRIPTION` in `styling-engine/attributes.js` as of 2026-09-02; the tagger schema and `/extract-pieces` both project them. **Defined 2026-09-02** — see [fit-on-body-definitions-spec.md](fit-on-body-definitions-spec.md). The axis is the garment's relationship to the BODY'S CONTOURS, not its looseness and not its outline (`silhouette` owns that): `silhouette: structured` + `fit_on_body: skims` is a normal combination for a tailored waisted coat. Gate-critical **for tops only** — `outfitValidation.js` splits the values into `BASE_LAYER_CLOSE_FITS` (`skims`/`clings_*`) and `BASE_LAYER_INCOMPATIBLE_FITS`, so a top on the wrong side is silently rejected as a base layer under a `needs_base: yes` garment. For every clothing category it is also stated to the model verbatim in image-generation and anchor prompts (`core.js`), so a wrong value instructs the renderer to draw the wrong shape. No consumer distinguishes `structured` from `hangs_straight` — both sit in the same set everywhere. |
...
> **Normalization, added 2026-09-01.** `fabric_category` is validated by
> `normalizeFabricCategory(value, category)` in `styling-engine/taggerMerge.js`, on both the tagger
> merge path and the `POST`/`PUT /pieces` write path — an out-of-list value becomes `null`. It was
> previously the only tagged enum in that file without a normalizer, which mattered because the
> value is read by material gates (wet-sensitive footwear, ventilated footwear, insulating
> material) that all ask "is this value in my set?". A value on **no** list therefore never matched
> and failed **open** on every one of them at once, silently. Found live: a shoe tagged
> `fabric_category: 'wool'` — a clothing value the prompt forbids for footwear — passed the wet gate
> on a 38°F rainy walking turn. Normalizing to `null` makes it read as *unknown*, which every
> consumer already handles, instead of as a silent pass. When the category is missing or
> unrecognized the value is checked against the union of the three lists rather than nulled: this
> rejects values on no list, it does not police category/value agreement.

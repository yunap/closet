...
| Field | Applies to | Valid values | AI-tagged | Editable | Source of truth |
|---|---|---|---|---|---|
| `fabric_category` | all | **top/bottom/dress/outerwear**: `jersey \| knit \| rib knit \| ponte \| sweatshirt fleece \| fleece \| cotton \| poplin \| linen \| linen blend \| rayon \| viscose \| modal \| silk \| satin \| crepe \| chiffon \| lace \| crochet \| wool \| cashmere \| denim \| twill \| canvas \| corduroy \| tweed \| velvet \| leather \| faux leather \| suede \| faux suede \| mesh \| technical/performance \| synthetic \| other` (35 values) — **shoes**: `leather \| suede \| patent \| canvas \| mesh \| synthetic \| textile \| rubber \| other` — **accessory**: `leather \| suede \| metal \| straw \| canvas \| synthetic \| textile \| rubber \| other` | yes, but see caveat | PieceForm, BatchAdd | `FABRIC_BY_CATEGORY[cat].fabricOptions`. **Tagger/UI mismatch, not yet fixed**: the tagger's schema (`prompts.js`) has one flat 35-value enum — the clothing list above — with no category branch at all, so it can never correctly tag a shoe's `leather`/`suede`/`patent`/`rubber` or an accessory's `metal`/`straw` as one of *their* narrower lists; it'll just pick the closest clothing-fabric word (e.g. `leather`, which happens to also be valid for shoes/accessory, or something that isn't in either shorter list and gets silently accepted since there's no `VALID_*` Set for this field). Same class of gap `silhouette`/`hem_finish` had before their 2026-08-14 rewrite; not fixed here. |
...
- `routes/ai.js`'s `/extract-pieces` schema still doesn't have `bottom_subtype`, `accessory_subtype`,
  ...
  split, category-conditional `length_hits_at`/`silhouette`/`hem_finish`, and the new `shoe_type`/
  `toe_shape` are synced as of 2026-08-14). Two independent tagger schemas remain a real drift
  risk; unifying them is worth doing at some point, not done here.

...
| [exposure-conditions-spec.md](exposure-conditions-spec.md) | Proposed 2026-09-03, no code. Supplies the input the band spec §9.1 declares missing. A Vienna trip shipped six cards — museum, hike, indoor dinner — all sized against the SAME `65°F high / 47°F low` envelope, so a down puffer was **required** on every 65°F city day: `needsRemovableCoolLayer` keys on `lowF` alone, a 5am trough nobody dresses for, and nothing bounds how warm the mandated layer may be. Owner ruling: the coat is needed, 47°F is the wrong size for it — wrong decision input, not wrong weather data. Census finding that corrects the band spec's "what exists today: nothing": a plan slot ALREADY carries occasion/activity/environment/date/location, all populated, all collapsed. **Field-level census against the standard thermal-comfort input model** (band spec §11 established the architecture; nobody had mapped Closet's fields onto it): a three-level metabolic proxy `none|walking|hiking` already exists on every slot and is thermally INERT — it drives shoes; wind is resolved and never reaches the thermal model. Genuinely missing is only exposure duration, ensemble insulation (band spec owns), and conditions-during-exposure as a sourced value. So acceptance case A is reachable from fields that exist TODAY, and daypart sourcing ships last. Defines `resolveExposureContext`; `needsRemovableCoolLayer` loses authority rather than being retuned; calendar season stays a separate axis (retires the weather-corroboration framing). Acceptance case A is the one that matters: same date and forecast, three slots, must not resolve to identical demand |

...
| [fiber-evidence-completeness-spec.md](fiber-evidence-completeness-spec.md) | Proposed. Why the black puffer read as uninsulated: `fiber_content` conflates "the fibres are X" with "the fibres I could see are X", and the care label's SHELL/LINING/FILL split has nowhere to land. The partial state is **already written** (10 pieces carry resolved fibres plus `"unknown"`) and read by nobody — the `[R1]` shape again. Proposes a derived `unknown\|partial\|complete` verdict over a stored column, one canonical fibre vocabulary replacing four agreeing copies, and field-consequence metadata that is not owned by `GATE_CRITICAL_FIELDS` |

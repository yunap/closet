...
| [cool-weather-tier-spec.md](cool-weather-tier-spec.md) | The 45-80°F blind spot: one cliff at `lowF<=45` does double duty as "needs a layer" and "needs a coat", with nothing between. Produced a puffer in all five cards of a 65/45 trip, then a sleeveless tank with no layer on a 65/48 one. Proposes a `cool` tier requiring a layer, not a coat, and gives `piece.season` its natural home. Proposed 2026-09-01, one threshold ruling open |

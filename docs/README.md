...
| [provisional-pieces-spec.md](provisional-pieces-spec.md) | Styling a garment you do not own. `status = 'active'` is the single predicate for wardrobe membership (28 server queries), so a `provisional` status excludes it everywhere by default and five sites are widened deliberately. Seven owner rulings open; specified 2026-08-20, not implemented |

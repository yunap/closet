...
| [freeform-prompt-cache-levers.md](freeform-prompt-cache-levers.md) | Where a freeform turn's tokens actually sit, and the five levers left. Lever 1 (tool-schema cache stability) implemented; lever 4 (volatile block) measured and declined at ~2%; the big instruction block audited (global material absolutes removed) and image budget specified; model tiering postponed by owner |

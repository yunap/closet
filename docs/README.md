...
| [search-propose-signal-inventory.md](search-propose-signal-inventory.md) | **Inventory complete 2026-09-03, no implementation.** Companion to model-facing-signal-inventory.md, scoped to the surfaces that document didn't cover: `search_wardrobe`, `suggest_slot_swaps`, `propose_outfit`, `generate_outfits`, and the no-tool full-stylist system prompt. Triggered by external review tracing thread_1788430055577's bad edit to a turn that never called `plan_outfit_set` — confirmed live from wire capture, not retroactive analysis, that `search_wardrobe` still emits `weatherFit`/`ruleFit` verdicts with no warmth/insulation facts behind them in either payload shape, sorts results by that same derived score (two independent sorts), and that `suggest_slot_swaps` uses it as the largest single weighted term in candidate scoring. Also finds thermal **policy**, not just per-turn verdicts, authored directly into the conversational system prompt (`stylistSystemTemplate`) — a blanket "no sleeveless indoors" rule and explicit instructions to obey `ruleFit`/`weatherFit`, present on every turn regardless of tool calls, which explains a no-tool turn making an unprompted thermal judgment. `generate_outfits`' own scoring path is flagged unconfirmed rather than assumed clean by similarity. Separately flags a real weather-provenance mislabeling (`stated_user` means "the argument was populated," not "the human said so") found in the same live capture, explicitly held out of scope per owner instruction since it didn't change what either turn received |
| [model-facing-signal-inventory.md](model-facing-signal-inventory.md) | **Inventory complete 2026-09-03, no implementation — live spending paused pending an owner decision.** Enumerates all 25 thermal/season/layer signals the model actually receives, verified against captured provider input rather than source. **19 of 25 are derived styling judgments; the garment thermal facts behind them are not in the payload at all** — `planWorkbenchPieceLine` emits `weight:`/`fabric:`/`sleeve_length:` but never warmth level, insulation, `interior_construction`, `season` or removability, so the model is told a trench is `discouraged` and cannot check why. Explains the Vienna oscillation (puffer on every outfit → no jacket at all) as mis-layering, not miscalibration: when the only thermal channel is a verdict, every calibration change swings the whole outfit. Demonstration case is the navy UPF sun hoodie — no insulation fact recorded, so absence reads as *a little* insulation, placing it `light` and therefore `preferred` for a 65°F October hike ahead of the trench. **Owner ruling: do not fix with a temperature rule.** Proposes facts-to-the-model/judgment-from-the-model, selection by RANGE not rank, and keeps the hourly-vs-sustained weather gap explicitly open |
| [thermal-comfort-band-spec.md](thermal-comfort-band-spec.md) | **IMPLEMENTED 2026-09-03 — the shipped owner of thermal amount.** `requiredThermalBand` compared against per-garment placement and outfit contribution, fed by `exposure.js`; ranking, model evidence, outfit adequacy and projection surfaces all read it, and no consumer derives thermal amount from a legacy cold flag. Began as the audit of `isCold` — one boolean carrying six distinct questions, which is why four correct weather fixes shipped in one day each beside the last. **Read §21.1 before touching the parallel contracts:** removable-layer need, transit coverage and outdoor capability keep their own triggers and must NEVER take band semantics — measured, because mapping band levels onto them would demand a removable layer at 75/60. Their calibration is a separate project. Final census: thermal_amount 0 · parallel_contract 13 · projection 12 · derivation 11 · producer 4 · non_thermal 4 (held throughout) |
...
| [trip-composition-parity-spec.md](trip-composition-parity-spec.md) | Why live capsule output reads more stylistically differentiated than trip output, even against a bigger/richer roster on a stronger provider: capsule composes through a dedicated single-shot call with unconditional full-roster images, full garment truth, and a composition-only prompt; trip composition runs inside the general conversational tool loop with none of the three. Not a provider-capability conclusion — providers are compared fairly within the trip path, they just don't explain the capsule-vs-trip gap. Proposes a trip-specific sibling of `composeCapsulePlanOnce`, reusing the infrastructure with a trip (not capsule) objective, with a hard invariant that composition changes who composes and what they see, never what `submit_plan_outfits` accepts. **Ratified 2026-09-04, ready to implement**: initial composition goes atomic, follow-up edits stay in the tool loop, no trip `/expand-capsule` equivalent |
| [cold-layer-exposure-trigger-spec.md](cold-layer-exposure-trigger-spec.md) | Live run 1337: a 65°F/45°F hiking slot lost both outfits to `NO_WARM_LAYER_FOR_COLD` because `isCold` keys on the raw overnight low, while the already-ratified exposure model rates the same slot `light` demand once waking-window smoothing and hiking's exertion are credited — exactly the question `cold-severity-spec.md` deferred. Adds one additive slot-level fact, `requiresWarmLayerForColdExposure`, computed once at the trip slot (never inside shared Contract C) and read by both `cold_layer_required` and `NO_WARM_LAYER_FOR_COLD` so they can't disagree; relaxes to false only when cold is non-severe, non-indoor, activity is exactly `hiking`, occasion is not explicitly `evening`, and a real waking-window temperature exceeds the existing threshold — no exertion-degree formula, no `requiredThermalBand` dependency, no global `isCold` change, no activity plumbing into shared infrastructure. **Ratified 2026-09-05, ready to implement** |
| [trip-roster-season-eligibility-spec.md](trip-roster-season-eligibility-spec.md) | Live thread: a winter Vienna, Virginia trip's roster spent 2 of 14 scarce slots on a warm-tagged pair of pants and a warm-tagged pair of sneakers, both `discouraged` per `seasonFitPieceAdvisory` but kept anyway per that advisory's own deliberate non-blocking ruling — the real defect is one stage earlier, at roster selection, not outfit selection. Ports capsule's existing hard season exclusion (`capsuleSeasonEligiblePool`) to trip roster selection, but category-aware where capsule's is category-blind: `bottom`/`shoes`/`dress` are hard-excluded when season-mismatched (no cocktail-dress exception), `top` stays advisory-only (may serve as an indoor layer), `outerwear` is excluded only when it is itself `warm`-tagged — a cool/cold-tagged layer is never excluded, on any trip, since it may be needed for one unexpectedly cold moment. **Ratified 2026-09-05, ready to implement** |
| [source-sensitive-insulating-credit-spec.md](source-sensitive-insulating-credit-spec.md) | Live 35°F thread + full-wardrobe census: `garmentWarmthLevel` gives the same flat `+2` credit whether "insulating" evidence came from a genuine recorded fill or from a face-fabric fiber name alone (`wool`/`fleece`/`cashmere`) — 23 of 34 real wardrobe pieces with fiber-only evidence reached `warm` purely from that bonus, confirmed by a clean isolated comparison (5 real short-sleeve cotton tops at `moderate`, a matched short-sleeve wool dress jumping two buckets to `warm` from the fiber term alone, no coverage/bare-cut confound). Ratifies a `+0.5` face-fiber credit (fill/construction evidence keeps `+2`), applicable to every category — not outerwear-only, per an earlier draft this one supersedes. Leaves `thermalMaterialVerdict`/`outerwearLayerPositivelyInadequate`/`outfitThermalContribution` untouched, and explicitly defers the sleeveless bare-cut penalty (`-2`) as a separate, not-yet-ratified calibration (`-0.5` is the leading candidate, unvalidated against real data — this wardrobe has no medium-weight sleeveless non-insulating pieces to check it against). **Ratified and implemented 2026-09-06** (fiber credit only — the sleeveless bare-cut penalty stays a separate, not-yet-ratified follow-up) |
| [thermal-ranking-source-sensitivity-and-overshoot-policy-spec.md](thermal-ranking-source-sensitivity-and-overshoot-policy-spec.md) | The fiber-credit fix above regressed the pinned Vienna incident test: a cardigan and a down puffer, now equidistant in opposite directions from the demand after the cardigan dropped a bucket, scored identically — and separately, a cardigan and a plain cotton jacket collapsed into the same named bucket, losing a real ranking distinction. Extracts `garmentWarmthScore` (the raw calculation behind `garmentWarmthLevel`, never exposed to the model) and adds `thermalRankingFit`, a shared ranking primitive consolidating two previously-independent, differently-calibrated scoring formulas (`weatherFitForPiece` and `buildVisualComposerRoster`'s own inline scoring). Same-bucket garments now rank apart by raw score; overshoot is weighted more than undershoot at equal raw distance from the demand's target (never touching `compareThermalFit`'s own adequacy classification). Two real bugs caught by the pinned tests before merge, not after: measuring against `demand.range` instead of `demand.level` (flattened Vienna via the coarse-uncertainty-widened tolerance), and inventing a numeric center for the unanchored `very warm`/`very light` extremes (produced a spurious tiny "undershoot" for a down puffer exactly matching severe-cold demand — `levelForRawScore`'s own rule against numeric distance beyond the verified range already forbade this). **Ratified and implemented 2026-09-06** |

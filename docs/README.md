...
| [layer-weight-ceiling.md](layer-weight-ceiling.md) | Every weather tier is a floor and nothing is a ceiling, so the warmest coat owned is always the safest answer — a puffer kept appearing on 65°F museum days across five runs and two providers. Heat already HAS a ceiling (`hot weather: insulating piece`); cool and cold have none. Recommends a ranking penalty rather than a gate, so supply-poor wardrobes degrade gracefully. Diagnosed 2026-09-01, ruling open |

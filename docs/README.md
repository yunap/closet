...
| [bounded-multi-context-continuity-spec.md](bounded-multi-context-continuity-spec.md) | A prose-only wardrobe answer (`search_wardrobe`, no cards) leaves no continuity for the next turn — three parallel "did we discuss pieces" mechanisms all gate on outfit-producing turns only, so "yes, do it" gets misrouted to `bounded_multi` and builds a disconnected roster. Fix informs the router's `contextSummary` and gives `full_stylist` the discussed pieces as continuation context requiring `view_pieces` re-verification. Implemented 2026-08-29; full live-shaped acceptance-criteria integration test still open (§9) |

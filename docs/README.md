...
| What does this `pieces` field mean / who fills it in? | [fit-on-body-definitions-spec.md](fit-on-body-definitions-spec.md) | Proposed. `fit_on_body` is asked for as a bare seven-value pipe-list with no definitions, while every comparable construction field got them in the 2026-08 passes. A live retag called a visibly waisted quilted jacket `hangs_straight` — the model saw the ribbed side panels and read them as "texture contrast" rather than waist shaping. Gate-critical for tops (base-layer compatibility) and stated verbatim to the image generator for everything else. Proposes definitions plus the three disambiguations that would have prevented it; no enum change, no migration |

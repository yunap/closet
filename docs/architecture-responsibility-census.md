...
| `buildPieceText`, `sharedGarmentEvidenceLine` / `sparseGarmentCatalogRow` (replaced composerPieceLineSuffix and the old `/ask` catalog line, 2026-09-15), wardrobe manifests, tool rows | Present piece facts to models and logs | `projection` | Composer prompts, `/ask` tools, trip composition, diagnostics |

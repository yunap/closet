...
- **Database Safety & Script Isolation (Mandatory)**: `wardrobe.db` is **live owner data**. Standalone scripts, probes, diagnostic CLI commands, and test harnesses must **NEVER** run against the root `wardrobe.db`. Any script importing `db.js` must explicitly isolate its database by setting `process.env.WARDROBE_DB_PATH` *before* importing `db.js` (e.g. using `createIsolatedDbSnapshot()` from `lib/databaseSafety.js` or a temporary fixture). When reading real wardrobe data in a diagnostic, copy `wardrobe.db` along with its WAL sidecars (`-wal`, `-shm`) to a temp directory — copying only the base file reads the past. `WARDROBE_ALLOW_LIVE_DB=1` is strictly forbidden in automated test runs, test suites (`NODE_ENV=test`), diagnostics, and speculative scripts; it is reserved exclusively for intentional, user-confirmed maintenance operations on the live database. Read and obey `docs/database-safety.md` before writing or executing any script that interacts with persistence.
